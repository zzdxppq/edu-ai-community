...
| 2026-04-21 | v1.4 | 模型对接实测适配：Qwen3-Embedding-8B 不可用→移除向量检索，改为关键词全文检索(zhparser)+Rerank方案；pgvector→zhparser；Qwen3-32B 上下文5500 tokens→检索Top-3、对话3轮、Prompt精简；FR9/FR35/FR41/NFR13/NFR14 相应调整；模型表更新真实部署信息 | 鲁班（架构师） |
...
- **FR9:** 系统基于知识库中的政策文件内容，通过关键词全文检索（PostgreSQL zhparser）+ 重排序（Qwen3-Reranker-8B）提供精准检索与答案生成。（注：Embedding服务不可用，向量检索暂不启用，后续可无缝升级）
...
- **FR35:** 系统在每次搜索平台数据后，自动将返回结果进行全文索引处理并写入知识库（PostgreSQL tsvector），以持续丰富检索语料、提升问答质量。（注：Embedding不可用，使用全文索引替代向量化）
...
- **FR41:** 系统须实现问答缓存机制，对高频热门政策问答进行缓存以降低模型API调用成本和响应延迟：
  - 对用户问题做文本标准化（去标点/分词/排序）后哈希匹配，命中则直接返回缓存答案。（注：Embedding不可用，语义相似度匹配降级为关键词标准化匹配，后续Embedding恢复可无缝升级）
  - 缓存按角色分组（不同角色同一问题的回答侧重点不同）
  - 知识库更新（新文件上传/条目修改）时自动清除全部缓存
  - 多轮追问、低置信度回答不使用缓存
...
- **NFR14:** 问答缓存命中率目标≥15%（注：Embedding不可用，降级为关键词匹配缓存，命中率低于语义缓存的30%目标；后续Embedding恢复可提升），缓存条目TTL默认7天，知识库变更时主动失效
...
| ~~Qwen3-Embedding-8B~~ | ~~文档和查询向量化~~ | ~~:22257~~ | **不可用**，已移除 |
...
- **数据库：** PostgreSQL + zhparser扩展（业务数据+全文搜索统一管理）。注：Embedding不可用，不使用pgvector，检索依赖全文搜索+Rerank
...
- **Epic 2: 知识库与文档处理引擎** — 文档解析管道（PDF/Word/表格/流程图）、全文索引入库、关键词检索+Rerank重排序、原文定位索引、平台数据实时同步与索引
...
**Epic Summary:** 构建文档解析管道（支持PDF/Word含表格和流程图）、文本切分与全文索引入库、关键词检索引擎（PostgreSQL zhparser + Qwen3-Reranker重排序）、原文定位索引，以及校共体网站平台数据实时同步与索引入库，为RAG问答提供高质量的知识检索基础。（注：Embedding不可用，检索方案为全文搜索+Rerank）
...
description: |
  ...
  按章-条-款语义切分，全文索引后入库PostgreSQL（zhparser分词+tsvector）。构建关键词
  检索引擎（PostgreSQL全文搜索+Qwen3-Reranker重排序），并建立片段→原文位置映射索引，
  支持原文跳转。同时对接校共体网站搜索API，实现平台数据（6大模块）的定时增量同步与
  全文索引入库，持续丰富知识库语料。（注：Embedding不可用，不使用pgvector向量检索）
    ...
    acceptance_criteria:
      ...
      - id: AC2
        ...
        scenario:
          ...
          when: "系统执行入库"
          then:
            - "片段文本通过PostgreSQL zhparser自动分词生成tsvector全文搜索索引"
            - "片段与元数据一起存储到PostgreSQL knowledge_chunks表"
            - "建立GIN索引加速全文检索"
        business_rules:
          - id: "BR-2.1"
            rule: "使用zhparser中文分词扩展，确保政策术语正确切词"
          - id: "BR-2.2"
            rule: "入库后通过触发器自动更新tsvector字段"
        error_handling:
          - scenario: "入库失败"
            code: "N/A"
            message: "N/A"
  ...
  - id: "2.3"
    ...
    acceptance_criteria:
      - id: AC1
        title: "关键词检索 + Rerank"
        scenario:
          given: "知识库已有全文索引的文档片段"
          when: "接收到用户查询"
          then:
            - "执行PostgreSQL全文检索（zhparser分词），召回Top-30候选"
            - "按用户角色过滤可见性"
            - "调用Qwen3-Reranker-8B（POST /v1/score）对候选逐条评分"
            - "按Rerank分数降序取Top-3最相关的文档片段（含元数据）"
        business_rules:
          - id: "BR-1.1"
            rule: "全文检索召回Top-30，Rerank精排后取Top-3（适配5500 token上下文）"
          - id: "BR-1.2"
            rule: "检索结果必须包含：片段文本、文件名、章节标题、页码、Rerank分数"
          - id: "BR-1.3"
            rule: "检索+Rerank总延迟控制在3秒内（30条Rerank可并发）"
        error_handling:
          - scenario: "Rerank API调用失败"
            code: "N/A"
            message: "N/A"
            action: "降级使用全文检索ts_rank分数排序取Top-3，记录日志"
    ...
    acceptance_criteria:
      ...
      - id: AC2
        ...
        scenario:
          ...
          when: "系统执行入库流程"
          then:
            - "将title + description拼接为文本，通过zhparser自动分词生成tsvector全文索引"
            - "文本与元数据（module、原始ID、title、publishTime、原始URL）一并写入knowledge_chunks表"
            - "基于module+原始ID去重，已存在记录仅更新（title/description/publishTime变更时）"
        business_rules:
          - id: "BR-2.1"
            rule: "入库文本格式：[模块类型] title\\n\\ndescription（description为空时仅用title）"
          - id: "BR-2.2"
...
请基于本PRD创建系统架构文档（Architecture），重点关注：RAG检索引擎设计（关键词检索+Rerank，无Embedding）、文档解析管道、脱敏管道、模型API集成方案（vLLM内网部署，Qwen3-32B上下文5500限制）、数据库Schema设计（PostgreSQL+zhparser全文搜索）、Redis会话管理、API接口规范。
