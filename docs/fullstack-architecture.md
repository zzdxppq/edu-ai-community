...
| 2026-04-21 | v1.1 | 模型对接实测适配：Embedding不可用→移除向量检索，改为关键词+Rerank方案；Qwen3-32B上下文5500→检索Top-3、对话3轮、Prompt精简；更新真实模型地址/ID/协议；移除pgvector依赖；语义缓存降级为关键词匹配缓存 | 鲁班（架构师） |

---

## ⚠️ 关键架构约束（2026-04-21 确认）

| 约束 | 影响 | 适配策略 |
|------|------|---------|
| **Qwen3-32B max_model_len = 5500** | 上下文极其有限 | 检索Top-3, 对话3轮, Prompt精简至400tok, 生成≤1600tok |
| **Embedding服务不可用** | 无法做向量语义检索 | 关键词全文检索(zhparser) + Reranker精排 |
| **无需API Key** | 简化认证 | 内网vLLM直连，无鉴权头 |
| **所有模型同一IP不同端口** | 网络简单 | 10.91.28.4:22214/22237/22205 |
...
| 数据库 | PostgreSQL 16 + zhparser（全文搜索） | — |
...
| 模块 | 职责 | 关键类 |
|------|------|--------|
| `rag` | RAG编排：查询改写→全文检索→重排→生成→引用校验 | `RagEngine`, `RagController`, `QueryRewriter`, `CitationValidator` |
...
> **架构约束**：Embedding 服务不可用，采用「关键词检索 + Rerank」方案。
> 对于 200-300 条初始数据规模，全文检索召回 + Reranker 语义精排可满足需求。

**关键词检索（PostgreSQL 全文搜索）**

```sql
-- 使用 zhparser 中文分词，召回 Top-30
...
**Rerank 精排（Qwen3-Reranker-8B）**

```
对 Top-30 候选逐一调用 POST /v1/score:
{
...
按 score 降序排列，取 Top-3 作为 LLM 上下文。
```

**Rerank 批量优化**：30 条候选可并发请求（线程池 / async），控制在 2-3 秒内完成。

### 3.3 问答缓存设计 (FR41 适配)

> **架构约束**：无 Embedding 服务，语义缓存降级为「关键词匹配缓存」。

```
缓存策略: 问题文本标准化后精确匹配 + 分词关键词模糊匹配
缓存键: role + normalized_question_hash
缓存值: { question, answer, citations, created_at }
TTL: 7天（配置项 app.cache.qa-ttl）

标准化流程:
  1. 去除标点/空格/语气词
  2. zhparser 分词 → 提取关键词 → 排序拼接
  3. MD5 哈希作为缓存键
...
失效策略:
  - 知识库变更（文件上传/条目编辑/删除）时清除全部缓存
  - 简单粗暴但安全，对200-300条知识库规模可接受
```

**Redis 存储结构:**

```
qa_cache:{role}:{question_hash}  →  Hash {
    question: "...",
    answer: "...",
    citations: "[...]",
    chunk_ids: "[...]",       # 关联的知识片段ID
...
> 注：后续若 Embedding 服务恢复可用，可无缝升级为语义缓存（余弦相似度 ≥ 0.95 匹配）。
...
> 注：无 Embedding 服务，跳过向量化步骤。检索完全依赖 PostgreSQL 全文搜索 + Rerank。
```
...
- 状态流转：`UPLOADING → PARSING → INDEXING → COMPLETED / FAILED`
...
| ~~Qwen3-Embedding-8B~~ | — | ~~:22257~~ | ~~向量化~~ | — | ❌ 不可用 |
...
> 注：无 Embedding 方法。检索依赖 PostgreSQL 全文搜索，无需向量化。
...
| Qwen3-Reranker-8B | 重试1次 → 降级使用全文搜索 ts_rank 分数排序 |
...
```sql
-- 注：无 Embedding 服务，不使用 pgvector 扩展
...
| `qa_cache:{role}:{question_hash}` | Hash | 问答缓存（关键词匹配） | 7d |
