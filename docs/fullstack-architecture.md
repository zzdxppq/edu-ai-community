...
| 模块 | 职责 | 关键类 |
|------|------|--------|
| `rag` | RAG编排：查询改写→检索→重排→生成→引用校验 | `RagEngine`, `RagController`, `QueryRewriter`, `CitationValidator` |
| `embedding` | 文本向量化（调用Qwen3-Embedding-8B） | `EmbeddingService` |
| `rerank` | 检索结果重排序（调用Qwen3-Rerank-8B） | `RerankService` |
...
两路检索结果合并后交给 Qwen3-Rerank-8B 重排序，取 Top-5。
...
```
你是"校共体AI助手"，服务于河南省城乡学校共同体建设。

当前用户角色：{role_name}
角色说明：{role_description}

## 回答规则
1. 严格基于【参考资料】回答，不得编造不在参考资料中的内容
2. 每个关键观点必须标注引用，格式为 [1]、[2] 等
3. 回答末尾列出引用来源清单，格式：[编号] 《文件名》第X章/条/款
4. 如果参考资料不足以回答问题，明确说明"暂无相关政策依据，建议咨询当地教育行政部门"
5. 语言风格：专业但易懂，适合教育工作者阅读
6. 回答长度控制在300-800字，要点分条阐述
7. 根据用户角色调整回复侧重点：
   - 区域教育管理人员：侧重宏观政策解读与管理决策
   - 校共体牵头校人员：侧重结对指导与经验分享
   - 成员校人员（城镇/乡村）：侧重实践案例与操作指引
   - 研究人员：侧重深度分析与文献引用
...
| 模型 | 用途 | 调用场景 | 超时 | 重试 |
|------|------|---------|------|------|
| Qwen3-32B | 主推理 | 问答生成、查询改写、NER脱敏、话题检测 | 60s | 1次 |
| Qwen3-VL-235B | 视觉理解 | 文档中图片/流程图识别 | 120s | 0次(跳过) |
| Qwen3-Embedding-8B | 向量化 | 文档切片、用户查询、语义缓存 | 30s | 3次 |
| Qwen3-Rerank-8B | 重排序 | 检索结果精排 | 15s | 1次(降级) |

### 6.2 统一调用封装

所有模型API采用 OpenAI 兼容接口格式，统一封装为 `LlmClient`：
...
app:
  ai:
    base-url: ${AI_API_BASE_URL:http://localhost:8080/v1}
    api-key: ${AI_API_KEY}
    models:
      chat: qwen3-32b
      embedding: qwen3-embedding-8b
...
| Qwen3-Rerank-8B | 重试1次 → 降级使用RRF融合分数排序 |
...
CREATE TABLE knowledge_chunks (
    ...
    embedding       vector(1024),            -- 向量（维度取决于模型）
...
|-------------|------|------|-----|
| `session:{user_id}:{conversation_id}` | List/String | 对话上下文（最近5轮） | 24h |
...
| `semantic_cache:{role}:{bucket}` | Hash | 语义缓存 | 7d |
...
- 上下文Token总量 ≤ 8000 tokens（超出时滑动窗口截断早期消息）
- TTL = 24小时
