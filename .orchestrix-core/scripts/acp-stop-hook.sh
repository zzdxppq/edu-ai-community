...
if [[ "$AGENT_ID" == "dev" || "$AGENT_ID" == "qa" ]]; then
    if [[ -n "$TRANSCRIPT_PATH" && -f "$TRANSCRIPT_PATH" ]]; then
        # H-7 fix: Only check last 2 lines (status line area) instead of 500 chars.
        # Previously, agent output content containing "Building..." or "Configuring..."
        # would falsely match and prevent the webhook from firing.
        TRANSCRIPT_TAIL=$(tail -c 500 "$TRANSCRIPT_PATH" 2>/dev/null | tail -2)

        # H-7 fix: Restrict pattern to line-start match (with optional ⏺ prefix)
        # to avoid matching "ing..." patterns embedded in agent output content.
        # Only matches Claude Code's status line format.
        if echo "$TRANSCRIPT_TAIL" | grep -qE '^\s*(⏺\s+)?[A-Z][a-z]+ing\s*(\.\.\.|\…)\s*$'; then
